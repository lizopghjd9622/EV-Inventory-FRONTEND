<template>
  <view class="sales-page">
    ...
    <view class="sales-page__main">
      <!-- 图标 -->
      <view class="sales-page__icon-wrap">
        <text class="sales-page__icon">📤</text>
      </view>

      <!-- 标题 -->
      <text class="sales-page__title">销售录单</text>
      <text class="sales-page__desc">长按按钮，语音创建销售单</text>

      <!-- 状态提示 -->
      <view class="sales-page__status-hint">
        <text
          v-if="store.status === RecordStatus.Recording"
          class="sales-page__status-text sales-page__status-text--recording"
        >
          🎙 正在录音，松开发送…
        </text>
        <text
          v-else-if="store.status === RecordStatus.Streaming"
          class="sales-page__status-text sales-page__status-text--streaming"
        >
          ⚡ 识别中，请稍候…
        </text>
      ...
      <!-- 提示 -->
      <view class="sales-page__tips">
        <text class="sales-page__tips-text">💡 说出品名、数量、单价，自动生成订单</text>
      </view>
    </view>

    <!-- 底部导航 -->
    <BottomTabBar active="sales" />
  </view>
</template>
...
.sales-page {
  min-height: 100vh;
  background: linear-gradient(180deg, #fff5f0 0%, #fef8f5 100%);
  display: flex;
  ...
  &__icon-wrap {
    ...
    margin-bottom: 32rpx;
    box-shadow: 0 12rpx 40rpx rgba(232, 65, 24, 0.3);
  }
